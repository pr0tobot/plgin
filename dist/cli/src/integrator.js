...
import { detectLanguageFromPath, listFilesRecursive } from './utils/fs.js';
...
export async function integratePack(params) {
    ...
    if (requiresAgentic) {
        ...
        const vulnerabilities = await params.agent.scanForVulns(allItems.map((item) => item.contents).join('\n'), targetLanguage);
        return {
            changeSet,
    ...
    const adapted = await params.agent.adaptPack(pack, process.cwd(), params.instructions);
    const vulnerabilities = await params.agent.scanForVulns(adapted.items.map((item) => item.contents).join('\n'), targetLanguage);
    return {
        changeSet: adapted,
        testsRun: !params.dryRun,
        vulnerabilities
...
function computeCompatibility(pack, targetLanguage) {
...
function inferPrimaryLanguage(pack) {
...
async function deriveProjectProfile(language) {
...
function hasLanguageExample(pack, language) {
    if (!pack.manifest.examples)
        return false;
    if (Array.isArray(pack.manifest.examples)) {
        return pack.manifest.examples.some((example) => example.language === language);
    }
    const entries = pack.manifest.examples.entries;
    if (Array.isArray(entries)) {
        return entries.some((entry) => entry.language === language);
    }
    return false;
}
